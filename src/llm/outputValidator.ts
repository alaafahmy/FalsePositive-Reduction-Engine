...
import { LLMOutput, ValidationError } from "../types";
...
function extractFieldByRegex(raw: string, key: string): string | undefined {
...
function normaliseValue(val: string, allowed: Set<string>): string {
  if (allowed.has(val)) { return val; }
  for (const a of allowed) {
    if (a.toLowerCase() === val.toLowerCase()) { return a; }
  }
  return val;
}

export function validateLLMOutput(raw: string): LLMOutput {
  if (!raw || raw.trim().length === 0) {
    throw new ValidationError("LLM returned an empty response.");
  }
  ...
  if (!parsed) {
    ...
    if (isVuln && san && atk && conf) {
      ...
      if (aMatch) { explanation = aMatch[1].replace(/\\n/g, " ").trim(); }
      else {
        const proseIdx = raw.search(/"Is Vulnerable"/);
        if (proseIdx > 0) { explanation = raw.slice(0, proseIdx).replace(/[*"{}]/g, "").trim(); }
      }
      ...
      throw new ValidationError(
        `All three parsing layers failed. Raw (first 400 chars): ${raw.slice(0, 400)}`
      );
    }
  }

  const normalise = (key: string, allowed: Set<string>): string => {
  ...
  if (!VALID_IS_VULN.has(isVuln)) {
    throw new ValidationError(
      `Invalid "Is Vulnerable" value: "${isVuln}". Must be "Yes" or "No". Raw (first 300): ${raw.slice(0, 300)}`
    );
  }
  if (!VALID_SANITIZATION.has(san)) {
    throw new ValidationError(
      `Invalid "Sanitization Found" value: "${san}". Must be "Yes", "No", or "Unsure".`
    );
  }
  if (!VALID_ATTACK.has(atk)) {
    throw new ValidationError(
      `Invalid "Attack Feasible" value: "${atk}". Must be "Yes" or "No".`
    );
  }
  if (!VALID_CONFIDENCE.has(conf)) {
    throw new ValidationError(
      `Invalid "Confidence" value: "${conf}". Must be "Low", "Medium", or "High".`
    );
  }
  ...
  return {
    "Explanation":         explanation,
    "Is Vulnerable":       isVuln as "Yes" | "No",
    "Sanitization Found":  san   as "Yes" | "No" | "Unsure",
    "Attack Feasible":     atk   as "Yes" | "No",
    "Confidence":          conf  as "Low" | "Medium" | "High",
  };
}

export function deriveClassification(output: LLMOutput): "FP" | "TP" {
