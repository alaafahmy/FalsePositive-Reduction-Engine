...
import { Alert, CWERules } from "../types";

// ---- CWE Micro-Rubric Database ----

const CWE_DATABASE: CWERules[] = [
  // ----------------------------------------------------------------
...
const CWE_INDEX = new Map<string, CWERules>(
  CWE_DATABASE.map((r) => [r.cweId, r])
);

export function getCweRules(cweId: string): CWERules {
  const normalised = normaliseCweId(cweId);
  return CWE_INDEX.get(normalised) ?? GENERIC_RULES;
}

export function enrichAlert(alert: Alert): CWERules {
  return getCweRules(alert.cwe);
}

function normaliseCweId(id: string): string {
...
export function formatCweRubric(rules: CWERules): string {
