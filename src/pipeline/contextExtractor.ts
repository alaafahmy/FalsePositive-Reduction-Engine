...
import { Alert, CodeContext, TraceStep } from "../types";
...
const METHOD_START_PATTERNS: RegExp[] = [
...
function renderLines(lines: string[], fromIdx: number, toIdx: number): string {
...
function resolveAbsPath(stepFile: string, workspaceRoot: string): string {
  if (path.isAbsolute(stepFile)) return stepFile;
...
function roleFromType(type: TraceStep["type"]): "source" | "intermediate" | "sink" {
  if (type === "source") return "source";
  if (type === "sink") return "sink";
  return "intermediate";
}

function getAnnotationType(step: TraceStep): string {
  const desc = step.description.toLowerCase();
  if (step.type === "source") return "SOURCE";
  if (step.type === "sink") return "SINK";
...
export function extractContext(
  ...
  if (trace.length === 1) {
    ...
    for (let i = 0; i < trace.length - 1; i++) {
      ...
      if (
        currentStep.file === nextStep.file &&
...
function deduplicateSnippets(
  snippets: InternalSnippet[],
  cache: Map<string, string[]>,
  workspaceRoot: string
): InternalSnippet[] {
  if (snippets.length === 0) return [];
  
  const result: InternalSnippet[] = [snippets[0]];
