...
class ResultsViewProvider {
            ...
            switch (message.command) {
                ...
                case "openAlertPanel":
                    if (this._results) {
                        ...
                        if (selectedAlert) {
                            dashboardPanel_1.DashboardPanel.createOrShow(this._extensionUri, this._results, selectedAlert);
                        }
                    }
                    break;
            }
    ...
    async exportReport(format) {
        ...
        const defaultUri = vscode.Uri.file(this._sarifPath
        ...
        try {
            ...
            vscode.window.showErrorMessage(`ZeroFalse: Export failed: ${err.message}`);
        }
    }
...
<body>
  ...
  <script>
    ...
    updateResults(results, sarifPath) {
        ...
        if (isNewAlert && newAlert) {
            ...
            if (dashboardPanel_1.DashboardPanel.currentPanel) {
                dashboardPanel_1.DashboardPanel.currentPanel.postMessage({ type: 'NEW_ALERT', alert: newAlert, total: results.length, fpCount: results.filter(r => r.classification === 'FP').length, tpCount: results.filter(r => r.classification === 'TP').length });
            }
        ...
        if (dashboardPanel_1.DashboardPanel.currentPanel && results.length > 0) {
            const currentSelected = dashboardPanel_1.DashboardPanel.currentPanel.currentSelectedAlert;
            let newSelectedAlert = results.find(r => r.alertId === currentSelected?.alertId);
            if (!newSelectedAlert) {
                newSelectedAlert = results[results.length - 1];
            }
            dashboardPanel_1.DashboardPanel.currentPanel.updateData(results, newSelectedAlert);
        }
    }
    setRunningState(isRunning) {
        ...
        if (isRunning) {
            if (this._view)
                this._view.webview.postMessage({ type: 'SCAN_STARTED' });
            dashboardPanel_1.DashboardPanel.createOrShow(this._extensionUri, [], undefined);
            if (dashboardPanel_1.DashboardPanel.currentPanel)
                dashboardPanel_1.DashboardPanel.currentPanel.postMessage({ type: 'SCAN_STARTED' });
        }
        else {
            if (this._view)
                this._view.webview.postMessage({ type: 'SCAN_COMPLETED' });
            if (dashboardPanel_1.DashboardPanel.currentPanel)
                dashboardPanel_1.DashboardPanel.currentPanel.postMessage({ type: 'SCAN_COMPLETED' });
        }
        this._updateHtml();
    }
    clearResults() {
        ...
        if (dashboardPanel_1.DashboardPanel.currentPanel) {
            dashboardPanel_1.DashboardPanel.currentPanel.postMessage({ type: 'CLEAR_ALL' });
        }
        this._updateHtml();
    }
    _updateHtml() {
...
ResultsViewProvider.viewType = "zerofalse.resultsView";
function escapeHtml(str) {
    if (!str)
        return "";
    return str
...
//# sourceMappingURL=resultsViewProvider.js.map