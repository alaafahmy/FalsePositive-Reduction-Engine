...
function resolveCwe(ruleId) {
    ...
    // 3. embedded CWE tag
    const cweMatch = ruleId.match(CWE_PATTERN) || ruleId.match(RULE_CWE_TAG);
    if (cweMatch) {
...
function buildTrace(result) {
    ...
    return flowLocations.map((fl, idx) => {
        ...
        const stepType = idx === 0 ? "source" : idx === total - 1 ? "sink" : "step";
        return {
            type: stepType,
 ...
 */
function parseSarif(report, workspaceRoot) {
    const alerts = [];
